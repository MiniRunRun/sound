...
function setupIntroAnimation() {
  ...
  spans.forEach(span => {
    ...
    const duration = text.length * 50; // 每个字符50ms
...
function setupDescriptionBox() {
  ...
  function updateDescriptionPosition(mouseX, mouseY, box) {
    ...
    // 获取尺寸 - 不修改opacity
    const boxWidth = box.offsetWidth || 300; // 使用默认值如果无法获取
    ...
    if (top < 20) top = 20;
    if (top + boxHeight > screenHeight - 20) top = screenHeight - boxHeight - 20;
